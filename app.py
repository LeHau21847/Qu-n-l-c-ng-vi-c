...
from flask import Flask, render_template, request, redirect, flash, url_for, Response, jsonify, send_file
...
app.secret_key = 'cskv_super_secret_key'
...
class HoSo(db.Model):
    ...
    ngay_cap_nhat_buoc = db.Column(db.DateTime, default=datetime.now) # --- TRACKING SLA ---
...
with app.app_context():
    db.create_all()
    # Migration for old database instances
    try:
    ...
    # Khởi tạo Ghi chú nhanh nếu chưa có
...
def index():
    ...
    return render_template('index.html', 
        danh_sach=danh_sach, loai_ho_so_list=loai_ho_so_list,
    ...
    if request.method == 'POST':
        try:
            ...
            if ma_ho_so and ten_nguoi_dan and loai_id:
                # Ép kiểu loai_id sang int để query chính xác
                ...
                if buoc_dau_tien:
                    new_hoso = HoSo(
                        ...
                        buoc_hien_tai_id=buoc_dau_tien.id,
                    ...
                    if old_hoso:
                        count = HoSo.query.filter_by(ma_ho_so=ma_ho_so.upper()).count()
                        flash(f"Đã nhận diện CÔNG DÂN CŨ ({ten_nguoi_dan.upper()}). Tiếp nhận hồ sơ số {count} hoàn tất!", "success")
                    else:
                        flash("Đã tiếp nhận hồ sơ thành công!", "success")
                else:
                    flash("Loại hồ sơ này chưa được cấu hình Bước xử lý. Hãy vào Cấu hình để tạo bước trước!", "warning")
            else:
                flash("Vui lòng điền đầy đủ các thông tin bắt buộc!", "warning")
        except Exception as e:
            db.session.rollback()
            flash(f"Lỗi hệ thống khi lưu hồ sơ: {str(e)}", "danger")
...
def update_hoso(id):
    ...
    # Kiểm tra an toàn hoso.buoc_hien_tai để tránh crash
    if hoso.buoc_hien_tai_id and hoso.buoc_hien_tai:
        ...
        # Nếu chưa có bước hoặc bước cũ bị xóa, tìm bước đầu tiên
        ...
        if buoc_dau:
            ...
            flash("Lỗi: Loại hồ sơ này chưa có cấu hình bước xử lý!", "danger")
    return redirect(url_for('index'))
...
def tra_lai_hoso(id):
    ...
    if not hoso.is_tra_lai:
        ...
        if ly_do:
            hoso.ghi_chu = (hoso.ghi_chu or "") + f"\n[TRẢ LẠI]: {ly_do}"
        db.session.commit()
        flash(f"Đã cập nhật trạng thái Trả lại cho hồ sơ {hoso.ma_ho_so}.", "warning")
    return redirect(url_for('index'))
...
def khoi_phuc_hoso(id):
    ...
    if buoc_dau:
        ...
        hoso.ngay_cap_nhat_buoc = datetime.now()
        db.session.commit()
        flash(f"Đã khôi phục hồ sơ {hoso.ma_ho_so} về lại bước khởi điểm.", "success")
    return redirect(url_for('index'))
...
def api_qr(id):
    hoso = HoSo.query.get_or_404(id)
    # Dùng version=None để auto-size, tránh tràn dữ liệu gây lỗi 500
...
def export_excel():
    ...
    # Sử dụng openpyxl để tạo file Excel mẫu mực
    ...
    header_fill = PatternFill(start_color="1E3A8A", end_color="1E3A8A", fill_type="solid") # Màu xanh navy
    ...
    for col_num, cell in enumerate(ws[1], 1):
        ...
        ws.column_dimensions[cell.column_letter].width = 22 # Tự động giãn cột tương đối
    ...
    for idx, hs in enumerate(danh_sach, 1):
        ...
        row = [
            ...
            hs.get_trang_thai_label(), 
            hs.ngay_tao.strftime('%d/%m/%Y %H:%M'),
            ngay_cn,
            hs.ghi_chu or ''
        ]
        ws.append(row)
...
def delete_hoso(id):
    ...
    # Lưu lại thông tin trước khi xóa để tránh crash sau khi commit
    ma_hoso_tam = hoso.ma_ho_so
    ten_hoso_tam = hoso.ten_nguoi_dan
    
    # 1. Xóa các tệp đính kèm vật lý trên ổ cứng để dọn dẹp bộ nhớ
    try:
...
def cau_hinh():
    ...
    return render_template('cau_hinh.html', 
        loai_ho_so_list=loai_ho_so_list, 
        active_loai=active_loai
    )
...
if __name__ == '__main__':
    # Nếu đang chạy bằng file .exe đã đóng gói thì tự gọi trình duyệt lên
    if getattr(sys, 'frozen', False):
